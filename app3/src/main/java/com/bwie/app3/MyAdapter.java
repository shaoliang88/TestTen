...
public class MyAdapter extends BaseExpandableListAdapter {
    ...
    private List<List<MyBean>> dataList;
    ...
    private void initData() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            ArrayList<MyBean> been = new ArrayList<>();
            for (int y = 0; y < 3; y++) {
                strings[y] = "商家" + i + "商品" + y;

                map.put(y, false);

                MyBean bean = new MyBean("100", "1");
                been.add(bean);
            }
    ...
    public View getChildView(final int groupPosition, final int childPosition,
        ...
        holder.zengjia.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                List<MyBean> been = dataList.get(groupPosition);
    ...
    public String getShopPrice() {
        ...
        for (int y = 0; y < childList.size(); y++) {
            ...
            for (Map.Entry<Integer, Boolean> entry : entries) {
                if (entry.getValue()) {
                    MyBean bean = dataList.get(y).get(entry.getKey());
